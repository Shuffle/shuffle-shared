...
func LoadSpecificApps(resp http.ResponseWriter, request *http.Request) {
	...
	if strings.Contains(tmpBody.URL, "github") || strings.Contains(tmpBody.URL, "gitlab") || strings.Contains(tmpBody.URL, "bitbucket") {
		...
		if tmpBody.ForceUpdate && project.Environment != "cloud" {
...
// Onlyname is used to
func IterateAppGithubFolders(ctx context.Context, fs billy.Filesystem, dir []os.FileInfo, extra string, onlyname string, forceUpdate bool) ([]BuildLaterStruct, []BuildLaterStruct, error) {
	var err error
	...
	for _, file := range dir {
		...
		case mode.IsRegular():
			...
			if filename == "Dockerfile" {
				...
				if len(allapps) == 0 {
					allapps, err = GetAllWorkflowApps(ctx, 0)
					if err != nil {
	...
	if len(extra) == 0 {
		if project.Environment == "cloud" {
			log.Printf("[AUDIT] Handling upload of %d apps in cloud", len(buildLaterFirst))
		} else {
