...
// Wazuh demo responses based on action type
func generateWazuhDemoData(actionName string) string {
	responses := map[string]string{
		"get_alerts": `{
			"alerts": [
				{"id": "12345", "severity": "High", "source_ip": "192.168.1.100", "description": "Suspicious login attempt", "timestamp": "2025-09-22T10:30:00Z"},
				{"id": "12346", "severity": "Medium", "source_ip": "10.0.0.45", "description": "Failed authentication", "timestamp": "2025-09-22T10:28:15Z"},
				{"id": "12347", "severity": "Critical", "source_ip": "203.0.113.45", "description": "Potential malware detected", "timestamp": "2025-09-22T10:25:30Z"}
			],
			"total_count": 3,
			"status": "success"
		}`,
		"query_logs": `{
			"logs": [
				{"timestamp": "2025-09-22T10:30:00Z", "level": "WARNING", "message": "Multiple failed login attempts from 192.168.1.100", "agent": "web-server-01"},
				{"timestamp": "2025-09-22T10:28:15Z", "level": "ERROR", "message": "Authentication failure for user 'admin'", "agent": "db-server-02"}
			],
			"query_time": "0.245s",
			"status": "completed"
		}`,
		"get_agent_status": `{
			"agents": [
				{"id": "001", "name": "web-server-01", "status": "active", "last_seen": "2025-09-22T10:30:00Z"},
				{"id": "002", "name": "db-server-02", "status": "active", "last_seen": "2025-09-22T10:29:45Z"},
				{"id": "003", "name": "file-server-01", "status": "disconnected", "last_seen": "2025-09-22T09:15:22Z"}
			],
			"total_agents": 3,
			"active_agents": 2
		}`,
	...
	if response, exists := responses[strings.ToLower(actionName)]; exists {
		return response
	}
	return `{"alert_id": "DEMO-12345", "severity": "High", "source_ip": "192.168.1.100", "description": "Security alert detected", "status": "active"}`
}

// Jira demo responses based on action type
func generateJiraDemoData(actionName string) string {
	responses := map[string]string{
		"create_issue": `{
			"id": "10042",
			"key": "SEC-123", 
		...
		"update_issue": `{
			...
			"fields": {
				...
				"assignee": {"displayName": "John Doe", "emailAddress": "john.doe@company.com"},
				"updated": "2025-09-22T10:35:00.000+0000"
			},
			"status": "updated"
		}`,
		"get_issue": `{
			...
			"fields": {
				...
				"assignee": {"displayName": "Security Team", "emailAddress": "security@company.com"},
				"created": "2025-09-22T10:30:00.000+0000"
			}
		}`,
		"add_comment": `{
			"id": "10100",
			"body": "Automated comment: Security alert has been processed and ticket created.",
			"author": {"displayName": "Shuffle Automation", "emailAddress": "automation@company.com"},
			"created": "2025-09-22T10:35:00.000+0000",
			"status": "added"
		}`,
	}
	...
	return `{"ticket_id": "DEMO-123", "key": "DEMO-123", "status": "Created", "assignee": "security-team", "summary": "Demo ticket created"}`
}

// Slack demo responses
func generateSlackDemoData(actionName string) string {
	responses := map[string]string{
		"send_message": `{
			"ok": true,
			"channel": "C1234567890",
			"ts": "1695384900.123456",
			"message": {
				"text": "High priority incident detected. Jira ticket SEC-123 created and assigned to security team.",
				"user": "U0123456789",
				"ts": "1695384900.123456"
			}
		}`,
	...
	return `{"ok": true, "channel": "C1234567890", "ts": "1695384900.123456", "status": "message_sent"}`
}
